...
def img2ascii(grey_image, char_list, scaled_img_width=53, scaled_img_height=7):
    ...
    ascii_img = [[None for i in range(scaled_img_width)]
    ...
    for i in range(scaled_img_height):
        for j in range(scaled_img_width):
            ...
            ascii_img[i][j] = char_list[min(
                int(brightness * char_list_length / 255), char_list_length-1)]
    return ascii_img
